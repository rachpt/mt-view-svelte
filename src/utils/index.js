...
export {
  debounce, debounceImmediate, throttle, sortMasonry, NEXUS_TOOLS, parseLocalStorage
  , handleMouseOver
  , handleMouseOut
  , handleMouseMove
}
...
function NEXUS_TOOLS() {
  ...
  document.body.addEventListener("mouseover", function (e) {
    const imgEle = get(_trigger_nexus_pic) == 0 ? e.target : e.relatedTarget;
  ...
  document.body.addEventListener("mousemove", function (e) {
    ...
    // 局部触发触发
    // @ts-ignore
    if (get(_trigger_nexus_pic) == 1 && e.target.matches(selector)) {
      handleMouseMove(e, imgEle);
    }
  });
